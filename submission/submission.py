...
# ...existing code...
    intro = (
        "\n"
        "╔" + "═" * 40 + "╗\n"
        "║" + f'{params["competition_name"]:^40}' + "║\n"
        "╠" + "═" * 40 + "╣\n"
        "║" + f'{"Server Submission Flag":^40}' + "║\n"
        "╠" + "═" * 40 + "╣\n"
        "║" + f'{"One flag per line please!":^40}' + "║\n"
        "╚" + "═" * 40 + "╝\n\n"
    )
...
# ...existing code...
    ...
    while True:
        ...
        try:
            ...
            writer.write(b'Invalid flag because is not encoding\n')
        ...
        try:
            ...
            writer.write(b'Invalid flag because invalid format\n')
            log('INFO', 'Flag %s rejected due to invalid format', repr(flag))
            metrics['flags_inv'].labels(client_net_no).inc()
            continue
        except flag_lib.InvalidFlagMAC:
            writer.write(b'Invalid flag because invalid MAC\n')
            log('INFO', 'Flag %s rejected due to invalid MAC', repr(flag))
            metrics['flags_inv'].labels(client_net_no).inc()
            continue
        except flag_lib.FlagExpired as e:
            
            writer.write(b'Flag has expired\n')
            log('WARNING', 'Flag %s rejected because it has expired since %s and now = %s', repr(flag),
                e.expiration_time,str(datetime.datetime.now(datetime.timezone.utc)))
            metrics['flags_old'].labels(client_net_no).inc()
            continue

        if protecting_net_no == client_net_no:
            writer.write(b' You cannot submit your own flag\n')
        ...
        try:
            ...
            try:
                ...
                writer.write(b'Could not find team\n')
